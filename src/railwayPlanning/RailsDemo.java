...
class Surface extends JPanel implements MouseListener {
	*//**
	...
	private boolean midCommand = false;
	private TrackSection target;
	...
	private void drawTracks(TrackSection ts, Graphics2D g2d) {
		...
		if (ts.getClass() == Switch.class) {
			...
			if (!trackID.contains(s.getExtraTrack().getTsID())) drawTracks(s.getExtraTrack(), g2d);

		} else {
			if (ts.isEndTrack()) {
				if (ts.isRightEnding()) {
					if (!trackID.contains(ts.getLeftTrack().getTsID())) drawTracks(ts.getLeftTrack(), g2d);
				} else {
					if (!trackID.contains(ts.getRightTrack().getTsID())) drawTracks(ts.getRightTrack(), g2d);
				}
			} else {
				if (!trackID.contains(ts.getLeftTrack().getTsID())) drawTracks(ts.getLeftTrack(), g2d);
				if (!trackID.contains(ts.getRightTrack().getTsID())) drawTracks(ts.getRightTrack(), g2d);
	...
	private void drawTrack(TrackSection ts, Graphics2D g2d) {
		...
		if (isThrown) {
			...
			if (isRightDirection) l.x1 += 40;
			else l.x2 -= 40;
			g2d.draw(l);
		} else g2d.draw(ts.getTrackGraphic());
	...
	private void placeTracks(TrackSection ts, int x, int y) {
		...
		if (ts.getClass() == Switch.class) {
			...
			if (!trackID.contains(s.getExtraTrack().getTsID())) {
				if (s.isRightDirection()) x += trackLengthStraight;
				else x -= trackLengthStraight;

				if (s.isTurnRight()) y += trackVerticalSpace;
				else y -= trackVerticalSpace;
	...
	private void placeTrain() {
		TrackSection curLoc = train.getLocation();
		Point drawLoc = curLoc.getPosAlongTrack(0.5);
		train.getTrainBox().setLocation(drawLoc.x - train.getTrainBox().width / 2, drawLoc.y - train.getTrainBox().height / 2);
	...
	public void mouseClicked(MouseEvent e) {
		...
		// for (Train tr : Train) {
		if (train.getTrainBox().contains(e.getPoint())) {
			System.out.println("you clicked THE TRAIN " + e.getPoint());
			if (midCommand) {
				SolutionCmd newCmd = new SolutionCmd(target, train);
		...
		for (TrackSection ts : tracks) {
			if (ts.getClass() == Switch.class) {
				Switch s = (Switch) ts;
				if (s.getSwitchLabelBox().contains(e.getPoint())) {
...
public class RailsDemo extends JFrame implements KeyListener, ActionListener {
	...
	*//** Handle the key-pressed event from the text field. *//*
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_G) System.out.println("gheter");
		repaint();
	}
